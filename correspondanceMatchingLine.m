function corsSSD = correspondanceMatchingLine(I1, I2, corners1, FMatrix, R, SSDth)
epiLine_dino02 = [];
for i = 1:size(corners1, 1)
    line_2 = linePts(FMatrix  *[corners1(i,:),1]', [1, size(I2, 2)], [1, size(I2, 1)]);
...
for i = 1:size(epiLine_dino02, 1)
    ...
    for j = 1:size(points, 1)
        if points(j, 2)-R > 0 && points(j, 2)+R < size(I2,1) && points(j, 1)-R > 0 && points(j, 1)+R < size(I2,2)
            window_2 = I2(points(j, 2)-R:points(j, 2)+R, points(j, 1)-R:points(j, 1)+R);
