...
for i = 1:size(toy_ori_img, 1)
   for j = 1:size(toy_ori_img, 2)
       max_intensity = max(toy_conv(i,j), max_intensity);
   end
end
...
car1_template = flip(car1_template, 1);
car1_template = flip(car1_template, 2);
...
car2_template = flip(car2_template,1);
% car1 overlap finding process
...
car3_template = imcrop(car_template_img, [90 280 1630 620]);
car3_template = imresize(car3_template, [box_width 70]);
car3_template = flip(car3_template,1);
car3_template = flip(car3_template,2);
...
car4_template = flip(car4_template,1);
...
car5_template = flip(car5_template,1);
